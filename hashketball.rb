...
def num_points_scored(player)
  game_hash.each do |team, value|
    game_hash[team][:players].each do |player_name, player_stats|
...
def player_numbers(team_name)
  ...
  game_hash.each do |team, value|
    if game_hash[team][:team_name] == team_name
      game_hash[team][:players].each do |player_name, player_stats|
        player_nums << game_hash[team][:players][player_name][:number]
      end
    end
  end
  player_nums
...
def big_shoe_rebounds
  ...
  game_hash.each do |team, value|
    game_hash[team][:players].each do |player_name, player_stats|
      if shoe_size(player_name) > largest_shoe_size
        largest_shoe_size = shoe_size(player_name)
        largest_shoe_owner = player_name
      end
    end
    game_hash[team][:players].each do |player_name, player_stats|
      if player_name == largest_shoe_owner
        return game_hash[team][:players][player_name][:rebounds]
      end
    end
  end
end
